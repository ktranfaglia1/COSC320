/* Kyle Tranfaglia
*  COSC320 - Homework01 - Ex.5
*  Last updated 09/08/23
*  This program creates a dynamic array of integer pointers to random integer values, sorts it, and times the process
*/
#include <chrono>
#include <iostream>
#include <cstdlib>
...
int main() {
    ...
    int* (*array) = new int*[size]; // Creates dynamic integer pointer array of size
    // Fills array with integer values between -1000 and 1000 inclusively in which a pointer is created to point to each value
    for (int i = 0; i < size; i++) {
        *(array + i) = new int((rand() % 10000) + 1);
    }
...
void insertion(T *A, int sz) {
    ...
    // For this particular sort, values are not moved, only pointers to the values and pointers must be dereferenced to compare values
    for (int i = 0; i < sz; i++) {
        int j = 0;
        T value = A[i];
...
bool sorted(T *A, int sz) {
    ...
    // For this particular sort checker, the pointers must be dereferenced to compare values
