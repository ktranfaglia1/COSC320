...
*  Templated Red-Black tree object Declaration and Implimentation - Code provided by Dr Spickler - added some functions
*/
#ifndef RBTREE_H_
#define RBTREE_H_
...
protected:
	...
	int calculateIPL(RBTreeNode<T>*, int);
	int calculateNodeCount(RBTreeNode<T>*);
...
template<class T>
int RBTree<T>::height(RBTreeNode<T>* nodePtr) const {
...
template<class T>
int RBTree<T>::calculateIPL(RBTreeNode<T>* nodeptr, int pathLength) {
    if (!nodeptr) {
        return 0;
    }
...
template<class T>
int RBTree<T>::calculateNodeCount(RBTreeNode<T>* nodeptr) {
    if (!nodeptr) {
        return 0;
    }
